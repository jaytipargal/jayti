#!/usr/bin/env python3
"""
EKA Agent — FAISS Retrieval Server (VPS deployment)
Memory-efficient: loads FAISS index + offset-index, reads docs on-demand.

Deployed on Vultr VPS (139.84.165.81) port 8100.
Paths adapted for /opt/eka-agent/faiss_index/
"""
import os
...
# ── Paths (VPS) ────────────────────────────────────────────────────────
INDEX_DIR = Path("/opt/eka-agent/faiss_index")
...
doc_offsets: Optional[np.ndarray] = None
doc_count: int = 0
